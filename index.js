      ...
      switch (args[0].toLowerCase()) {
        ...
        case "changeusername":
          if(whitelistBotMod.indexOf(message.author.id) > -1) {
            ...
            client.user.setUsername(message.content.substring(modPrefix.length+args[0].length)).then(user => message.channel.send('My new nickname is `` '+user.username+' `` !')).catch(message.channel.send('Something went wrong while changing my username to `` '+user.username+' `` !'));
          } else {
